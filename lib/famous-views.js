...
if (typeof(famous) !== 'undefined') {
  log.info("Starting up.  famous global found while loading package, great!");
  FView.startup();
}
else
  Meteor.startup(function() {
    if (typeof(famous) !== 'undefined') {
      log.info("Starting up.  famous global found during Meteor.startup()");
    	FView.startup();
    } else {
      log.info("No famous global available in Meteor.startup().  Call FView.startup() when appropriate.");
    }
  });
