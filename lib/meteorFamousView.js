...
MeteorFamousView = function(blazeView, options, noAdd) {
  ...
  if (parent) {
    ...
    parent = FView.byId("mainCtx");
    if (!parent) {
      // should never happen?  means we couldn't work around old behaviour
      log.warn("child has no famousContext parent and mainCtx not ready");
      parent = { node: FView.mainCtx, children: [] };
    }
  }
