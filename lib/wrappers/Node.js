var NodeView = new Blaze.Template('Famous.Node', function() {
  var blazeView = this;
...
function attrUpdate (key, value, oldValue, data, firstTime) {
  var fview = this;
  ...
  switch(key) {
    case 'size':
      ...
      for (i=0; i < value.length; i++) {
        ...
        if (cur[0] === 'R') {
          sizes['P'][i] = parseFloat(cur[1]);
          sizes['D'][i] = cur[2] ? parseFloat(cur[2]) : 0;
          usedSizes['P'] = true;
          usedSizes['D'] = true;
        } else if (cur[0] === 'renderSize') {
...
FView._attrUpdate = attrUpdate; // for testing

NodeView.created = function() {
  var blazeView = this.view;
  var fview = new MeteorFamousView(fviewParentFromBlazeView(blazeView), null, 'Node',
    parentDetails(blazeView));
  blazeView._fview = fview;
  fview.blazeView = blazeView;
  fview.node = fview.parent.node.addChild();
  fview._attrUpdate = attrUpdate; // for testing
...
Blaze.registerHelper('Node', NodeView);
