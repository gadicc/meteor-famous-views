    ...
    template: {
      ...
      onCreate: function(_class) {
        return function() {
          ...
          fview.components[_class.prototype.className] = comp;      
          fview.domElement = comp.instance; // shortcut

          var nodePath = fview.node.getId();
          var nodeContext = nodePath.split('/', 1)[0];
          ...
          fview._loadedDomRenderer = function() {
            ...
            // support 0.5.2
            if (FamousEngine.compositor.getContext)
              context = FamousEngine.compositor.getContext(nodeContext);
            else
              context = FamousEngine.compositor.getOrSetContext(nodeContext);

            if (!context)
              return null;

            // support 0.5.2
            if (context.getDOMRenderer)
              domRenderer = context.getDOMRenderer();
            else
              domRenderer = context.DOMRenderer;

            domRenderer.loadPath(nodePath);
          ...
          var blazeRenderComponent = fview.node.addComponent({
            onUpdate: function(time) {
              ...
              if (domRenderer && domRenderer._target &&
                ...
                fview.components[_class.prototype.className].renderedView =
