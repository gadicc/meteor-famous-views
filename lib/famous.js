...
function famousCreated() {
  ...
  compView._view = famousCmp.views[this.data.view] || { name: this.data.view };
...
famousCmp.famousView = Template.__create__(
  ...
  function() {        // Blaze.View "renderFunc"
    ...
    for (key in data) {
      if (view.hasRendered && data[key] == tpl.data[key])
        continue;
      tpl.data[key] = data[key];

      // view and modifier specific actions
      if (compView._modifier && compView._modifier.attrUpdate)
        compView._modifier.attrUpdate.call(compView,
          key, data[key], data, !view.hasRendered);
      if (compView.view && compView.view.attrUpdate)
        compView.view.attrUpdate.call(compView,
          key, data[key], data, !view.hasRendered);
    }
