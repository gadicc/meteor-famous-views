  ...
  if (options.view=='Surface' || div.innerHTML.trim().length) {
    ...
    */
    ...
    if (autoSize) {
      ...
      log.warn(fview.surfaceClassName + ': size="[undefined,auto"] is deprecated.'
        + 'Since Famo.us 0.3.0 '
...
function famousCreated() {
  ...
  // this will catch the case where no attributes are given
  this.data = blazeView.parentView.name == "with" ? this.data : {};

  // and probably this can be updated to rely on the above.  maybe.
  ...
  */
  ...
  if (this.data.view /* != 'Surface' */) {
    ...
    var registerable = FView._registerables[this.data.view];
    if (!registerable)
      throw new Error('Wanted view/modifier "' + this.data.view + '" but it doesn\'t exists.'
        + ' Try FView.registerView/Modifier("'+ this.data.view +'", etc)');
  ...
  if (!this.data.modifier && !fview.modifier &&
      (this.data.origin || this.data.translate || this.data.transform
      || (this.data.size && !node.size)))
    this.data.modifier = 'Modifier';

  // Allow us to prepend a modifier in a single template call
  if (this.data.modifier) {

    fview._modifier = FView._registerables[this.data.modifier];
