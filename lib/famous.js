...
//  var calledWithAttributes = 
//  console.log(this.data, Blaze._parentData(0));

  console.log(this, arguments);
  console.log('this.data', this.data);
  console.log('      pd0', Blaze._parentData(0));
  console.log('      pd1', Blaze._parentData(1));

  // don't re-use parent's data/attributes, don't mutate data object
  var inNewDataContext = blazeView.parentView.name == "with";
  ...
  if (data.size && _.isString(data.size) && data.size.substr(0,1) != '[')
    throw new Error('[famous-views] size="' + data.size + '" is deprecated, please use '
      + 'size="['+ data.size + ']" instead');
  ...
  */
  ...
  if (data.view /* != 'Surface' */) {
    ...
    if (!registerable)
      throw new Error('Wanted view/modifier "' + data.view + '" but it doesn\'t exists.'
        + ' Try FView.registerView/Modifier("'+ data.view +'", etc)');
  ...
  */
  ...
  var dataContext = data.data
    || inNewDataContext && Blaze._parentData(1, true)
    || Blaze._parentData(0, true);
