...
/*
 * We need to make two changes from original Tracker.flush:
 *
 ...
 *    and restart after on next cycle.
 */

Tracker.FViewFlush = function(_opts) {
  if ((_opts && _opts._fviewResume)) {
    log.debug('Tracker.FViewFlush -- resuming');
  } else {
    if (inFlush)
      throw new Error("Can't call Tracker.flush while flushing");
    ...
    try {
      while (pendingComputations.length ||
        ...
        if (pendingComputations.length) {
          log.debug('Tracker.FViewFlush -- splitting after ' +
            (Date.now() - currentTime) + 'ms, ' +
            pendingComputations.length + ' pending computations');
          return Tracker.FViewFlush({_fviewResume: true}); // defer is inside
        }
