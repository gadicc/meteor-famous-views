 ...
 *
 * 1) Defer Tracker.flush, instead of running as soon as possible
 * 2) Limit how long we can rerun computations for, before we break
 *    and restart after on next cycle.  We should monitor the famo.us
 *    defer loop to make to a) work out how long we can run for and
 *    b) make sure we don't do our next cycle in same famou.us cycle
 */

// Based on https://github.com/meteor/meteor/blob/1f3741cba202e132fc68eca3bbf828b6b0d2b7b8/packages/tracker/tracker.js
// commit 1f3741cba2, first released in tracker 1.0.5 in Meteor 1.0.3
 ...
 * actual function.  Our only change is to comment out "Tracker = {}" to
 * avoid changing exports
 */
...
// caution and hope for the best.
 ...
 var MAX_DEFER_FRAME_TIME = 2.5;

// Our real code changes come here

FView.ready(function() {
  var Engine = famous.core.Engine;

  log.debug('Tracker.flush -- replacing');
  Tracker.flush = function(_opts) {
    if ((_opts && _opts._fviewResume)) {
      log.debug('Tracker.flush -- resuming');
    } else {
      if (inFlush)
        throw new Error("Can't call Tracker.flush while flushing");
    ...
    // fviews change #1 - defer
    Engine.defer(function (_opts) {
      inFlush = true;
      willFlush = true;
      throwFirstError = !! (_opts && _opts._throwFirstError);
      ...
      try {
        while (pendingComputations.length ||
          ...
          // recompute all pending computations
          while (pendingComputations.length && // fviews#2
              (Date.now() - currentTime) < MAX_DEFER_FRAME_TIME) {
            var comp = pendingComputations.shift();
            comp._recompute();
          }

          // fviews#2: we didn't complete
          if (pendingComputations.length) {
            log.debug('Tracker.flush -- splitting after ' +
              (Date.now() - currentTime) + 'ms, ' +
              pendingComputations.length + ' pending computations');
            return Tracker.flush({_fviewResume: true}); // defer is inside
          }
