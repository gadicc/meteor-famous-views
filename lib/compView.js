...
var compViews = {};
var compViewsCount = 0;

CompView = function(component, options, noAdd) {
  this.id = options.id || ++compViewsCount;
  compViews[this.id] = this;
...
FView.fromBlazeView = famousCmp.dataFromView = function(view) {
  while ((view=view.parentView) && !view.famousView);
  return view ? view.famousView : undefined;  
}

FView.fromTemplate = famousCmp.dataFromTemplate = function(tplInstance) {
  return this.dataFromView(tplInstance.__view__);
}

FView.fromElement = famousCmp.dataFromElement = function(el) {
  var view = Blaze.getElementView(el);
...
famousCmp.dataFromComponent = function(component) {
  log.warn("famousCmp.dataFromComponent has been deprecated.  Please use 'FView.fromBlazeView' instead.");
